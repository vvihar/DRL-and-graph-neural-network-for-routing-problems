    ...
    for veh_number, r in enumerate(routes):
        ...
        qv = ax1.quiver(
            ...
            label="R{}, N({}), C {} / {}, D {:.2f}".format(
                veh_number,
                len(r),
                int(total_route_demand[0])
                if round_demand and round_demand
                else total_route_demand,
...
def vrp_matplotlib(Greedy=True):
    node_ = np.loadtxt(
        "VRP/test_data/vrp100_test_data.csv", dtype=np.float64, delimiter=","
    )
    demand_ = np.loadtxt(
        "VRP/test_data/vrp100_demand.csv", dtype=np.float64, delimiter=","
    )
    capacity_ = np.loadtxt(
        "VRP/test_data/vrp100_capacity.csv", dtype=np.float64, delimiter=","
    )
