    ...
    def run_train(self, data_loader, batch_size, valid_loder):
        ...
        for i in range(self.epoch):
            ...
            for batch_idx, batch in enumerate(data_loader):
                ...
                x, attr, capacity, demand = (
                    x.view(batch_size, n_nodes, 2),
                    attr.view(batch_size, n_nodes * n_nodes, 1),
                    capacity.view(batch_size, 1),
                    demand.view(batch_size, n_nodes, 1),
                )
                batch = batch.to(device)
                actions, log_p = self.agent.old_polic.act(
                    batch, 0, self.steps, batch_size, self.greedy, False
                )
                ...
                for i_batch in range(self.batch_size):
                    ...
                    memory.capacity.append(capacity[i_batch])
                ...
                if (batch_idx + 1) % time_Space == 0:
                    ...
                    print(
                        "  Batch %d/%d, reward: %2.3f,took: %2.4fs"
...
def train():
    ...
    for (
        ...
        data_loder = create_data(n_nodes, data_size, batch_size)
        valid_loder = create_data(n_nodes, valid_size, batch_size)
