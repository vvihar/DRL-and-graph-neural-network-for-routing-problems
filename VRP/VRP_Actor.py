...
class Decoder1(nn.Module):
    ...
    def forward(self, encoder_inputs, pool, capacity, demand, n_steps, T, greedy=False):
        ...
        dynamic_capacity = capacity.view(encoder_inputs.size(0), -1)  # bat_size
        demands = demand.view(
            encoder_inputs.size(0), encoder_inputs.size(1)
        )  # （batch_size,seq_len）
        ...
        for i in range(n_steps):
            ...
            """decoder_input = torch.cat([pool,_input,dynamic_capacity], dim=-1)
            decoder_input  = self.fc(decoder_input)"""
            # -----------------------------------------------------------------------------------------------------------
            if i == 0:
...
class Model(nn.Module):
    ...
    def forward(self, datas, n_steps, greedy=False, T=1):
        ...
        actions, log_p = self.decoder(x, pooled, capacity, demand, n_steps, T, greedy)
        return actions, log_p
