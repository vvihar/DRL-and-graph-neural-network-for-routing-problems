...
def train():
    ...
    for (
        ...
        data_loder = create_data(n_nodes, data_size, batch_size=batch_size)
        valid_loder = create_data(n_nodes, 10000, batch_size=batch_size)
        print("Data creation completed")

        actor = Model(
            3, hidden_node_dim, 1, hidden_edge_dim, conv_layers=conv_layers
        ).to(device)
