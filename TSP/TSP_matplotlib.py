...
def plot_tsp(xy, tour, ax1, i, Greedy):
    ...
    if Greedy:
        ...
        ax1.set_title(
            "Greedy-{} Nodes, Total Length {:.2f}".format(len(tour), lengths[-1]),
        ...
        ax1.set_title(
            "Sampling1280-{} Nodes, Total Length {:.2f}".format(len(tour), lengths[-1]),
            family="Times New Roman",
        )
    plt.show()
...
def tsp_matplotlib(Greedy=True):
    node_ = np.loadtxt(
        "./test_data/tsp100_test_data.csv", dtype=np.float, delimiter=","
    )
    ...
    agent = Model(2, 128, 1, 16, conv_layers=4).to(device)
    agent.to(device)
    folder = "trained"
