...
def plot_vehicle_routes(
    ...
    routes = [
        r[r != 0]
        for r in np.split(route.cpu().numpy(), np.where(route.cpu().numpy() == 0)[0])
        if (r != 0).any()
    ]
    print(routes)
    routes = [
        np.array([0, 91, 9, 85, 83, 21, 89, 82, 16, 90, 15, 67, 47, 49, 7, 1]),
        np.array([1, 84, 35, 33, 34, 17, 94, 96, 42, 1]),
        np.array([1, 52, 60, 69, 95, 6, 11, 87, 2, 66, 0]),
        np.array([0, 79, 98, 38, 27, 64, 40, 14, 37, 20, 101, 1]),
        np.array([1, 53, 72, 57, 8, 59, 43, 54, 80, 1]),
        np.array([1, 24, 3, 18, 77, 74, 58, 1]),
        np.array([1, 13, 100, 4, 28, 97, 88, 86, 55, 76, 0]),
        np.array([0, 93, 51, 44, 48, 12, 70, 63, 39, 0]),
        np.array([0, 30, 68, 78, 61, 46, 22, 71, 0]),
        np.array([0, 29, 99, 73, 0]),
        np.array([0, 92, 50, 26, 45, 56, 41, 75, 65, 62, 0]),
        np.array([0, 32, 5, 31, 23, 81, 25, 10, 36, 19, 0]),
    ]
    ...
    if Greedy:
        ax1.set_title(
            "Greedy;总路线数:{};总距离:{:.2f}".format(len(routes), cost.item()),
            family="SimSun",
            size=20,
        )
    else:
        ax1.set_title(
            "OR-tools;总路线数:{};总距离:{:.2f}".format(len(routes), 14.77),
            family="SimSun",
            size=20,
        )
...
def vrp_matplotlib(Greedy=True):
    ...
    data = Data(
        ...
        capacity=torch.tensor(capacity_[x]).unsqueeze(-1).float(),
    )
    datas.append(data)
