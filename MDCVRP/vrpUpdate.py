...
def update_state(
    demand, dynamic_capacity, selected, num_depot, c=20
):  # dynamic_capacity(num,1)
    ...
    return dynamic_capacity.detach()  # (bach_size,1)


def update_mask(demand, capacity, selected, mask, i, num_depot):
    go_depot = torch.zeros((selected.size(0)), dtype=torch.bool).to(device)
    for j in range(num_depot):
        go_depo = selected.squeeze(
            -1
        ).eq(
            j
