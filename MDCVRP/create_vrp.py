...
def create_instance(num, n_nodes=100, random_seed=None):
    ...
    CAPACITIES = {10: 2.0, 20: 3.0, 50: 4.0, 100: 5.0}
    num_depot = n_nodes % 10
    demand = np.random.randint(
        1, 10, size=(n_nodes - num_depot)
    )  # Demand, uniform integer 1 ... 9
    demand = np.array(demand) / 10

    for j in range(num_depot):
        demand = np.insert(demand, 0, 0.0)
    capacity = CAPACITIES[n_nodes - num_depot]
    return datas, edges, demand, capacity  # demand(num,node) capacity(num)


"""a,s,d,f = create_instance(2,21)
print(d,f)"""


def create_data(n_nodes, num_samples=10000, batch_size=32):
    edges_index = []
    ...
    for i in range(num_samples):
        node, edge, demand, capacity = create_instance(num_samples, n_nodes)
        data = Data(
...
def reward1(static, tour_indices, n_nodes):
    ...
    t_len, _ = torch.cat(
        [
            tour_len.sum(1).unsqueeze(-1).detach(),
            tour_len_1.sum(1).unsqueeze(-1).detach(),
            tour_len_2.sum(1).unsqueeze(-1).detach(),
            tour_len_3.sum(1).unsqueeze(-1).detach(),
        ],
        -1,
    ).min(-1)
